...
echo "Training PredNet with fc, sigmoid, tanh, send_acts, local_grad on CCN dataset"
echo "Learning rate is 0.0001, no steps in scheduler"
echo "Layer lambdas at 1.0, 1 layer"
...
--layer_lambdas 1.0 1.0 1.0 \
--learning_rate 0.0001 \
--lr_steps 0 \
--results_dir ../results/train_results \
--out_data_file train_prednet_fc_sigmoid_tanh_acts_lg_3l_ccn_lr0p0001_llam1p0.json \
--checkpoint_path ../model_weights/train_prednet_fc_sigmoid_tanh_acts_lg_3l_ccn_lr0p0001_llam1p0.pt \
--checkpoint_every 2 \
--record_E True \
